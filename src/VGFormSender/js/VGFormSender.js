...
class VGFormSender extends VGSender {
	...
	alert(vgSender, data, status) {
		...
		if (_this.settings.alert.params.type === 'modal' && 'modal' in _this.settings) {
	...
	alertModal(vgSender, data, status) {
		...
		if (data !== null) {
			...
			let modal = new _this.settings.modal('#' + _this.classes.alert.modal, {backdrop: true, keyboard: true, focus: true});
			modal.show();
		}
