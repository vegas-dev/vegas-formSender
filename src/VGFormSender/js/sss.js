...
const setParams = function (params, arg) {
	return mergeDeepObject(params, arg);
}
...
class VGSender {
	constructor(form, arg = {}) {
		...
		if (!form) {
			...
			this.form = form;
			this.settings = setParams(defaultParams, arg);

			let data = [].filter.call(form.attributes, function(at) { return /^data-/.test(at.name); });

			for (let val of data) {
				if (val.name === 'data-alert-type' && val.value) this.settings.alertParams.type = val.value;
				if (val.name === 'data-alert') this.settings.alert = val.value !== 'false';
				if (val.name === 'data-validate') this.settings.validate = val.value !== 'false';
				if (val.name === 'data-json-parse') this.settings.jsonParse = val.value !== 'false';
				if (val.name === 'data-redirect' && val.value) this.settings.redirect = val.value;
